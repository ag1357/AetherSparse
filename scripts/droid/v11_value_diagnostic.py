...
def build_diagnostic(
    ...
    for item in ordered_residuals:
        ...
        gold_documents = {
            ...
            if (key := _document_key(str(evidence["document_id"]))) is not None
        }
        retrieved_documents = {
            key
            for evidence in ranked
            if (key := _document_key(str(evidence.get("document_id", "")))) is not None
        }
    ...
    if packs:
        for key in list(unavailable):
            unavailable[key] = "available only for replicas whose tier pack was supplied"
