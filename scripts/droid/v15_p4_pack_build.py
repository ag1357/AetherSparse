...
def build_evidence(
    path: Path, entity_index: dict[str, int], supplement_path: Path | None = None
) -> tuple[bytes, dict]:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="optional self-article lead occurrences appended after the "
        "backlink stream (v15_p4_selflead_supplement.py output)",
    )
    ...
    evd_blob, evd_stats = build_evidence(
        address / "occurrences.jsonl.gz", entity_index, arguments.selflead_supplement
    )
    ...
    manifest = KnowledgePackManifest.create(
        ...
        compiler_identity=f"aethersparse v15_p4_pack_build v1 @ {arguments.repo_head}",
        regions=regions,
    )
    ...
    provenance = {
        "schema_version": "aethersparse.v15-p4-pack-provenance.v1",
        "selflead_supplement_sha256": (
            hashlib.sha256(arguments.selflead_supplement.read_bytes()).hexdigest()
            if arguments.selflead_supplement
            else None
        ),
