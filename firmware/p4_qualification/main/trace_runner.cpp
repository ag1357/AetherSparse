...
/* ------------------------------------------------------------------ */
/* phase 4: raw storage characterization through the deployment path   */

static bool phase4_storage(void) {
  RegionFile *evd = pack_region_evidence();
  uint8_t *buffer = (uint8_t *)heap_caps_malloc(1 << 20, MALLOC_CAP_SPIRAM);
  uint32_t *lat = (uint32_t *)heap_caps_malloc(3000 * 4, MALLOC_CAP_INTERNAL);
  uint64_t *offsets = (uint64_t *)heap_caps_malloc(3000 * 8, MALLOC_CAP_INTERNAL);
  if (!buffer || !lat || !offsets) return false;

  /* sequential read: first 256 MiB of the evidence region, 1 MiB chunks */
  const uint64_t seq_bytes = 256ull << 20;
  int64_t start = esp_timer_get_time();
  bool ok = true;
  for (uint64_t off = 0; off < seq_bytes && ok; off += (1 << 20)) {
    ok = region_read(evd, off, buffer, 1 << 20);
    if ((off & 0x3FFFFFF) == 0x3FFFFFF) vTaskDelay(1);
  }
  double seconds = (double)(esp_timer_get_time() - start) / 1e6;
  printf("MEAS {\"phase\":\"storage.device\",\"metric\":\"sequential_read\","
         "\"bytes\":%llu,\"seconds\":%.3f,\"mb_per_s\":%.2f,\"ok\":%s}\n",
         (unsigned long long)seq_bytes, seconds,
         ok ? (double)seq_bytes / seconds / 1e6 : 0.0, ok ? "true" : "false");

  /* random 4 KiB, two passes over identical offsets (cold, then warm) */
  const int samples_4k = 3000;
  uint64_t span = evd->length - PACK_PAGE;
  for (int i = 0; i < samples_4k; i++) {
    uint64_t raw = ((uint64_t)esp_random() << 32) | esp_random();
    offsets[i] = (raw % (span / PACK_PAGE)) * PACK_PAGE;
  }
  for (int pass = 0; pass < 2; pass++) {
    int64_t pass_start = esp_timer_get_time();
    for (int i = 0; i < samples_4k; i++) {
      int64_t op_start = esp_timer_get_time();
      region_read(evd, offsets[i], buffer, PACK_PAGE);
      lat[i] = (uint32_t)(esp_timer_get_time() - op_start);
      if ((i & 0x1FF) == 0x1FF) vTaskDelay(1);
    }
    double pass_seconds = (double)(esp_timer_get_time() - pass_start) / 1e6;
    char name[40];
    snprintf(name, sizeof(name), "random_4kib_read_%s", pass == 0 ? "cold" : "warm");
    emit_latency_stats("storage.device", name, lat, samples_4k);
    printf("MEAS {\"phase\":\"storage.device\",\"metric\":\"%s\",\"iops\":%.1f}\n",
           name, samples_4k / pass_seconds);
  }

  /* random 64 KiB */
  const int samples_64k = 800;
  const int chunk = 64 * 1024;
  uint64_t span64 = evd->length - chunk;
  int64_t r64_start = esp_timer_get_time();
  for (int i = 0; i < samples_64k; i++) {
    uint64_t raw = ((uint64_t)esp_random() << 32) | esp_random();
    uint64_t offset = (raw % (span64 / chunk)) * chunk;
    int64_t op_start = esp_timer_get_time();
    region_read(evd, offset, buffer, chunk);
    lat[i] = (uint32_t)(esp_timer_get_time() - op_start);
    if ((i & 0x3F) == 0x3F) vTaskDelay(1);
  }
  double r64_seconds = (double)(esp_timer_get_time() - r64_start) / 1e6;
  emit_latency_stats("storage.device", "random_64kib_read", lat, samples_64k);
  printf("MEAS {\"phase\":\"storage.device\",\"metric\":\"random_64kib_read\","
         "\"mb_per_s\":%.2f,\"iops\":%.1f}\n",
         (double)samples_64k * chunk / r64_seconds / 1e6, samples_64k / r64_seconds);

  /* sequential write: fresh 64 MiB file in free space, then deleted */
  const uint64_t write_bytes = 64ull << 20;
  memset(buffer, 0xA5, 1 << 20);
  FILE *tmp = fopen("/sdcard/.qual_tmp.bin", "wb");
  if (tmp) {
    int64_t write_start = esp_timer_get_time();
    for (uint64_t off = 0; off < write_bytes; off += (1 << 20)) {
      if (fwrite(buffer, 1, 1 << 20, tmp) != (1 << 20)) break;
      vTaskDelay(1);
    }
    fflush(tmp);
    fsync(fileno(tmp));
    double write_seconds = (double)(esp_timer_get_time() - write_start) / 1e6;
    fclose(tmp);
    remove("/sdcard/.qual_tmp.bin");
    printf("MEAS {\"phase\":\"storage.device\",\"metric\":\"sequential_write\","
           "\"bytes\":%llu,\"seconds\":%.3f,\"mb_per_s\":%.2f}\n",
           (unsigned long long)write_bytes, write_seconds,
           (double)write_bytes / write_seconds / 1e6);
  } else {
    printf("MEAS {\"phase\":\"storage.device\",\"metric\":\"sequential_write\","
           "\"status\":\"UNAVAILABLE\"}\n");
  }
...
static bool phase6_trace(void) {
  ac_int8_policy_v2 policy = make_policy();
  Pager *pager = pager_create(1u << 20); /* reference cache size */
  ...
  emit_latency_stats("addressing.device", "query_wall_1MiB", query_lat, query_lat_n);
  PagerStats stats;
  pager_stats(pager, &stats);
  printf("MEAS {\"phase\":\"replay.device\",\"seconds\":%.3f,\"cases_ok\":%u,"
  ...
  printf("MEAS {\"phase\":\"replay.device\",\"verdict\":\"%s\"}\n",
         logical_pass ? "LOGICAL_PASS" : "LOGICAL_MISMATCH");
  free(policy_lat);
  free(query_lat);
  free(cand_buf);
  pager_destroy(pager);
